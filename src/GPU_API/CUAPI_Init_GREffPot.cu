...
void CUAPI_Init_GREffPot()
{
   ...
   int     NBin        = Phi_eff.NBin;
   double *Radius      = Phi_eff.Radius;
...
// compute the location of edge
   Edge[0] = 0.0;
   for ( int i=1; i<NBin; i++ )   Edge[i] = ( Phi_eff.LogBin ) ? sqrt( Radius[i - 1] * Radius[i] )
                                                               : 0.5*( Radius[i - 1] + Radius[i] );
   Edge[NBin] = ( Phi_eff.LogBin ) ? SQR ( Edge[NBin - 1] ) / Edge[NBin - 2]
                                   : 2.0 * Edge[NBin - 1]   - Edge[NBin - 2];

   int Exitcode = CUPOT_SetConstMem_GREffPot( Phi_eff.Data, Edge, Phi_eff.Center, r_max2, NBin );
   if (  Exitcode != 0  )
